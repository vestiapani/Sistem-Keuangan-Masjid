...
export default function NotifikasiPage() {
  ...
  const notifs = [...transactions]
    .sort((a, b) => b.id - a.id)
    .slice(0, 10)
    .map((t) => ({
      id: t.id,
      type: t.type,
      title: t.type === 'Pemasukan' ? `${t.cat} Masuk` : `Pengeluaran: ${t.cat}`,
    ...
    <div className="max-w-4xl space-y-6">
      ...
      <div className="space-y-3">
        {notifs.length === 0 ? (
          ...
          notifs.map((n) => (
            <Card key={n.id} className="hover:border-[#0F4C3A] transition-colors cursor-pointer">
              <CardContent className="p-5 flex items-start space-x-4">
                <div className={`p-2 rounded-full ${n.type === 'Pemasukan' ? 'bg-[#E8F3F0] text-[#0F4C3A]' : 'bg-rose-50 text-rose-500'}`}>
                  {n.type === 'Pemasukan' ? <ArrowDownRight size={20} /> : <ArrowUpRight size={20} />}
                </div>
