...
// Ambil transaksi terbaru (donasi + pengeluaran) lintas semua periode, untuk notifikasi
export async function getNotifikasiTerbaru(limit = 10) {
  const { data: donasiAll, error: e1 } = await supabase
    .from("donasis")
    .select("id, jumlah_dana, tanggal_donasi, nama_donatur, kategori, created_at")
    .eq("status_verifikasi", "verified")
  ...
  const { data: pengeluaranAll, error: e2 } = await supabase
    .from("pengeluarans")
    .select("id, jumlah_pengeluaran, tanggal_pengeluaran, keperluan, kategori, created_at")
    .order("created_at", { ascending: false })
    .limit(limit);
  if (e2) throw e2;
