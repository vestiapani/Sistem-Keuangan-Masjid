...
// ============================================================
// Simpan / upsert laporan per periode
// ============================================================
export async function simpanLaporan({
  ...
  // Get current user - jika tidak ada session, throw error yang jelas
  const {
    data: { user },
    error: authError,
  } = await supabase.auth.getUser();
  if (authError || !user) {
    throw new Error("Harus login untuk menyimpan laporan.");
  }
  ...
  if (error) {
    ...
    if (error.code === "42501") {
      throw new Error(
        "Akses ditolak. Pastikan kebijakan RLS tabel laporans mengizinkan insert untuk pengguna terautentikasi.",
      );
    }
    throw error;
  }
